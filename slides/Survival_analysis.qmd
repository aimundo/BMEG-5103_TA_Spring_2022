...
sc<-data$Survival_date
i<-1
for (val in 1:length(sc)){
    
    test<-seq(from=0,to=sc[val])
...
#data<-data %>%
#    mutate(Patient=factor(row_number()))
#colnames(data)<-c("Survival_time","Censoring_status","Age","Treatment")
...
data %>%
    ...
    geom_point(aes(x=Survival_date,y=Patient,shape=Category,color=Category),size=4)+
